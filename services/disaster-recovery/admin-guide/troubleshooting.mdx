---
title: "Disaster Recovery Admin Troubleshooting"
description: "Diagnose and resolve Disaster Recovery administrator-level issues — initial sync failures, unexpected automatic failovers, runbook script errors, and site connectivity problems."
sidebarTitle: "Troubleshooting"
---

## Overview

This page covers administrator-level Disaster Recovery diagnostics — issues requiring access to
...
[Disaster Recovery User Guide — Troubleshooting](/services/disaster-recovery/user-guide/troubleshooting).

<Note>
  **Prerequisites**
  - Administrator credentials with the `dr-admin` role
  - Access to both primary and DR site the deployment console instances
...
<AccordionGroup>
  ...
  <Accordion title="Automatic failover triggered unexpectedly" icon="bolt">
  ...
  <Accordion title="Site connectivity test fails" icon="network-wired">
    ...
    **Resolution**:
    ...
    - Check that Disaster Recovery agent processes are running on both sites by reviewing the agent
      status indicator in **Disaster Recovery → Sites → [Site]**
    - If the agent is stopped, restart it via the deployment console on the affected site
    - Check agent logs for TLS certificate errors — certificates may have expired
  </Accordion>

  <Accordion title="Plan status shows DEGRADED" icon="circle-xmark">
    ...
    **Resolution**:
    - For lag issues: see [Disaster Recovery User Guide — Troubleshooting](/services/disaster-recovery/user-guide/troubleshooting)
...
All diagnostic operations are performed through the Disaster Recovery Dashboard:
...
|-----------|--------------|
| Disaster Recovery controller logs | **Monitoring → Log Explorer** → `service: disaster-recovery-controller` |
| Disaster Recovery agent logs (primary) | **Monitoring → Log Explorer** → `service: disaster-recovery-agent AND site: primary-dc1` |
| Disaster Recovery agent logs (DR) | **Monitoring → Log Explorer** → `service: disaster-recovery-agent AND site: dr-site-a` |
...
- A failover event log shows an internal Disaster Recovery controller error (not a script or connectivity error)
...
<CardGroup cols={2}>
  <Card title="Disaster Recovery User Guide — Troubleshooting" href="/services/disaster-recovery/user-guide/troubleshooting" color="#bf9667">
