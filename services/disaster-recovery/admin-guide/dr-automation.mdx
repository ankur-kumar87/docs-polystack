...
| Variable | Value |
|----------|-------|
| `DR_PLAN_NAME` | Name of the recovery plan |
| `DR_SITE` | Name of the recovery site (DR site) |
| `DR_GROUP_NAME` | Name of the current resource group |
| `DR_RESOURCE_IDS` | Space-separated list of recovered resource IDs |
| `DR_EVENT_ID` | Unique identifier for this failover event |
| `DR_RECOVERY_POINT` | Timestamp of the recovery point used |
...
<AccordionGroup>
  <Accordion title="DNS update after recovery" icon="globe">
    ...
    # DR_RESOURCE_IDS and DR_SITE are provided by the Disaster Recovery runtime
    # Resource IPs are resolved from the recovery metadata
    for id in $DR_RESOURCE_IDS; do
      # Parse recovery metadata for IP and hostname
      ip=$(echo "$DR_RECOVERY_METADATA" | jq -r ".resources[\"$id\"].ip")
      hostname=$(echo "$DR_RECOVERY_METADATA" | jq -r ".resources[\"$id\"].hostname")
    ...
    echo "DNS update complete for group: $DR_GROUP_NAME"
    ```
  </Accordion>
  <Accordion title="Service registry update" icon="list">
    ...
    # DR_RESOURCE_IDS and DR_RECOVERY_METADATA are provided by the Disaster Recovery runtime
    for id in $DR_RESOURCE_IDS; do
      ip=$(echo "$DR_RECOVERY_METADATA" | jq -r ".resources[\"$id\"].ip")
      service=$(echo "$DR_RECOVERY_METADATA" | jq -r ".resources[\"$id\"].tags[\"service-name\"]")
  ...
  <Accordion title="Pre-failover notification" icon="paper-plane">
    ...
    curl -sf -X POST "$WEBHOOK_URL" \
      ...
      -d "{
        ...
        \"plan\": \"$DR_PLAN_NAME\",
        \"site\": \"$DR_SITE\",
        \"event_id\": \"$DR_EVENT_ID\",
        \"recovery_point\": \"$DR_RECOVERY_POINT\"
      }"

    echo "On-call notified for failover event $DR_EVENT_ID"
    ```
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Disaster Recovery User Guide — DR Testing" href="/services/disaster-recovery/user-guide/test-dr" color="#bf9667">
