...
<AccordionGroup>
  <Accordion title="Token validation failures across services" icon="circle-xmark" defaultOpen>
    ...
    **Diagnose**: Verify key synchronization across all nodes. All nodes must have
    ...
    ```bash title="Force key rotation via ironcore-ansible"
    ironcore-ansible deploy --tags keystone-fernet-rotate
    ```
  ...
  <Accordion title="All services returning 401 Unauthorized" icon="lock">
    ...
    **Resolution**: Restart the Identity service via the deployment console if containers are stopped:
...
<AccordionGroup>
  <Accordion title="LDAP users cannot authenticate" icon="network-wired">
  ...
  <Accordion title="LDAP users not appearing in user list" icon="users">
    ...
    **Resolution**: Verify the LDAP configuration matches your directory schema:
    ...
    ```
    Update the configuration via the deployment console globals and redeploy if attributes are incorrect.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Service Catalog" href="/services/identity/service-catalog" color="#bf9667">
