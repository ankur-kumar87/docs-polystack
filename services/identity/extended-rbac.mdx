---
title: "Extended RBAC"
description: "Fine-grained role-based access control for Polystack Platform — privileges, custom roles, tag conditions, and per-action enforcement on every Dashboard action."
sidebarTitle: "Extended RBAC"
---

## Overview

**Polystack Extended RBAC** is the per-action access control layer of the Polystack Platform.
It builds on the platform's existing identity and policy stack (Polystack Identity tokens,
...
<Note>
  **Prerequisites**
  - An administrator account on the Polystack Dashboard with the `rbac_admin` privilege
    (or the built-in `admin` role)
  - The Polystack Platform identity layer enabled (default on every cluster)
  - The RBAC gateway enabled in **the deployment console → Configuration** for cluster-wide
...
<iframe
  ...
  title="How to Edit Permissions on a Custom Role with Extended RBAC on Polystack"
...
|-------|-----------------|----------------------|
| **1. Authentication** | Who can sign in (password, TOTP MFA, federation) | Polystack Identity, [User Center](/services/dashboard/user-guide/user-center) |
...
that go through the Polystack Dashboard's extension APIs.

The gateway is enabled through **the deployment console** — there is no other supported
configuration path.

<Steps titleSize="h3">
  <Step title="Open the deployment console → Configuration" icon="gear">
    From the deployment console, navigate to **Configuration**.
  </Step>
  ...
  <Step title="Run reconfigure" icon="play">
    From the deployment console → Operations, run a **reconfigure** for the Dashboard service.
...
<AccordionGroup>
  <Accordion title="Compute — Instances" icon="server">
    ...
    **Script execution endpoints** — Polystack has no general "exec arbitrary command
  ...
  <Accordion title="Compute — Snapshots, Images, Flavors, Keypairs, Server Groups, Templates" icon="layer-group">
  ...
  <Accordion title="Network — Networks, Routers, Floating IPs, Security Groups, Ports, Load Balancers, DNS" icon="network-wired">
  ...
  <Accordion title="Optimization (Resource Optimizer)" icon="rotate-right">
  ...
  <Accordion title="Container Infrastructure — Clusters, Cluster Templates" icon="microchip">
...
<Info>
  Some Polystack-developed actions (Clone, Convert to Template, Adjust Resources, Browse
...
| **Create Role** (primary) | Opens the create-role wizard. The role is created in Polystack Identity AND a matching Custom-role record is created in Extended RBAC — there is no separate step |
| **Edit** (row) | Edit name and description |
| **Delete** (row) | Removes the role from both Polystack Identity and Extended RBAC in one transaction. Disabled for System roles |
...
<Steps titleSize="h3">
  ...
  <Step title="Toggle privileges in the matrix" icon="table-cells">
  ...
  <Step title="Preview Effective Rules" icon="flask">
...
<AccordionGroup>
  <Accordion title="VM Power User — power on / off but cannot resize or edit" icon="power-off">
...
<Steps titleSize="h3">
  ...
  <Step title="Privilege lookup" icon="magnifying-glass">
...
<AccordionGroup>
  <Accordion title="Start with built-in roles, add Custom roles only for exceptions" icon="layer-group">
  ...
  <Accordion title="Preview effective rules before you save" icon="flask">
  ...
  <Accordion title="Keep admin bypass on for break-glass" icon="triangle-exclamation">
  ...
  <Accordion title="Forward audit log to long-term storage" icon="box-archive">
...
<CardGroup cols={3}>
  <Card title="Roles" href="/services/identity/roles" color="#bf9667">
    Manage Polystack Identity roles — create, assign, implied-roles
  </Card>
  <Card title="Policy Management" href="/services/identity/policy-management" color="#bf9667">
    Service-level policy YAML hardening
  </Card>
  <Card title="Multi-Factor Auth" href="/services/identity/multi-factor-auth" color="#bf9667">
