...
description: "Configure Fernet key rotation, token lifetime, and expiration policies for Polystack Identity."
...
Polystack Identity uses Fernet tokens by default — stateless, symmetric-key-encrypted tokens
...
<Tip>
  ...
  you need to validate tokens outside of Polystack (e.g., in a sidecar proxy or API gateway).
</Tip>
...
<Steps titleSize="h3">
  <Step title="Configure rotation schedule" icon="clock">
    the deployment console manages Fernet key rotation automatically via a scheduled cron job.
  ...
  <Step title="Rotate keys manually" icon="rotate-right">
...
<Warning>
  ...
  nodes cause token validation failures. the deployment console's rotation playbook handles
...
Token lifetime is configured in the deployment console globals. Shorter lifetimes improve security
