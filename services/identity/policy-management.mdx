...
description: "Customize RBAC policies to control which roles can perform each API operation across Xloud services. Create custom roles and map them to service-level."
...
<Steps titleSize="h3">
  ...
  <Step title="Write the policy override file" icon="file-code">
    ...
    ```yaml title="/etc/xavs/nova/policy.yaml — allow network-operator to list servers"
    ...
    ```yaml title="/etc/xavs/neutron/policy.yaml — allow network-operator to manage ports"
...
<Tabs>
  ...
  <Tab title="Networking (Neutron)" icon="network">
    ```yaml title="/etc/xavs/neutron/policy.yaml"
  ...
  <Tab title="Block Storage (Cinder)" icon="hard-drive">
    ```yaml title="/etc/xavs/cinder/policy.yaml"
  ...
  <Tab title="Image (Glance)" icon="image">
    ```yaml title="/etc/xavs/glance/policy.yaml"
  ...
  <Tab title="Identity (Keystone)" icon="fingerprint">
    ```yaml title="/etc/xavs/keystone/policy.yaml"
