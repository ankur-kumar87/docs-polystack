...
Each Polystack service reads its policy file from inside its container. Overrides are placed in `/etc/ironcore/<service>/` on the host and deployed by XDeploy:
