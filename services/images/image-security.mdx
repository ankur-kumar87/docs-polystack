...
description: "Configure image signing, property protections, and public image access controls for Polystack Image Service."
...
The Polystack Image Service security configuration covers three areas: image signature
...
Polystack Image Service supports image signature verification using certificates stored in
Polystack Key Management. When enabled, the compute service verifies the image signature
...
<Steps titleSize="h3">
  ...
  <Step title="Upload image with signature" icon="shield-halved">
  ...
  <Step title="Enable verification in Compute" icon="gear">
...
<Steps titleSize="h3">
  <Step title="Create the property protections configuration" icon="file-code">
    ```ini title="/etc/ironcore/glance/property-protections.conf"
    ...
    [x-image-meta-property-polystack_base_image]
    create = admin
    read = @
    update = admin
    delete = admin
...
```yaml title="/etc/ironcore/glance/policy.yaml — restrict public image creation"
...
<AccordionGroup>
  <Accordion title="Signature verification enabled" icon="shield-halved" defaultOpen>
  ...
  <Accordion title="Property protections configured" icon="lock">
    ...
    (`polystack_base_image`, `hw_firmware_type`) require admin to modify.
  </Accordion>
  ...
  <Accordion title="Audit public and community images" icon="magnifying-glass">
