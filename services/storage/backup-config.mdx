...
<Note>
  **Prerequisites**
  ...
  - Access to the deployment console for infrastructure-level configuration changes
...
<Warning>
  Only one backup backend is active at a time. All settings go in the `[DEFAULT]` section of `cinder.conf`. Apply changes through the deployment console — do not edit `cinder.conf` directly on production nodes.
</Warning>
...
<Tabs>
  ...
  <Tab title="GlusterFS" icon="network-wired">
...
In Ironcore deployments, backup backend settings are configured through the deployment console and applied via Ansible. Do not edit `cinder.conf` directly on production nodes.
...
<Tabs>
  <Tab title="Dashboard" icon="gauge">
    <Steps titleSize="h3">
      ...
      <Step title="Restore the backup" icon="rotate-left">
