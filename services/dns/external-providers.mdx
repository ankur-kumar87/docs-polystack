...
description: "Configure Polystack DNS as a Service against external authoritative DNS backends — what's actively tested upstream, what's in tree but untested, and how to integrate enterprise DDI platforms."
...
Polystack DNS as a Service uses Designate to manage DNS zones and records on behalf
...
All backend configuration lives in `designate.conf`. In Ironcore deployments, this
file is managed by Ansible — set parameters via the deployment console globals and run
`ironcore-ansible deploy --tags designate` to apply changes.
...
<Tabs>
  <Tab title="Dashboard" icon="desktop">
    <Steps titleSize="h3">
      ...
      <Step title="Add records" icon="pen-to-square">
...
<AccordionGroup>
  ...
  <Accordion title="Records not appearing on external DNS server" icon="triangle-exclamation">
  ...
  <Accordion title="Custom driver fails to load at startup" icon="circle-xmark">
...
<CardGroup cols={2}>
  ...
  <Card title="Designate Support Matrix (upstream)" href="https://docs.openstack.org/designate/latest/admin/support-matrix.html" color="#bf9667">
