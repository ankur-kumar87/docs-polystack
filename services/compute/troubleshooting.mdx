...
description: "Resolve common Polystack Compute issues — instances stuck in BUILD/ERROR, migration failures, scheduling errors, and quota issues."
...
This guide covers the most common operational issues encountered in Polystack Compute
...
<AccordionGroup>
  <Accordion title="Instance stuck in BUILD status" icon="clock">
    ...
    **Diagnosis**:
    ...
    ```bash title="Check Compute Agent logs on the target host (via the deployment console terminal)"
    ...
    **Common causes and resolutions**:
    ...
    |---------------------|-----------|
    | `Image download failed` | Verify Polystack Image Service reachability from the compute node |
    ...
    | `Volume attachment failed` | Check Polystack Block Storage service health |
  ...
  <Accordion title="Instance in ERROR state" icon="circle-xmark">
  ...
  <Accordion title="Live migration fails" icon="arrow-right-arrow-left">
    ...
    **Common errors and resolutions**:
    ...
    |--------------|-----------|-----------|
    | `guest CPU doesn't match specification: missing features` | CPU microarchitecture difference between hosts | Configure a common CPU baseline model on all hosts via the deployment console under **Compute → Advanced Settings → CPU Compatibility** |
  ...
  <Accordion title="No valid host found (scheduling failure)" icon="server">
    ...
    **Common causes**:
    ...
    | Cause | Resolution |
    |-------|-----------|
    | All hosts at vCPU or RAM capacity | Scale out the cluster or increase over-commit ratios via the deployment console |
    ...
    <Warning>
      ...
      state. Trigger a resource reconciliation through the deployment console under **Compute →
      Diagnostics → Reconcile Inventory**.
    </Warning>
  </Accordion>

  <Accordion title="Console connection refused" icon="desktop">
    ...
    **Resolution**:
    ...
    2. If the console proxy service is `down`, restart it through the deployment console under
