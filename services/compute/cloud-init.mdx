...
Polystack Compute supports automatic execution of customer-supplied scripts
...
consume the same Polystack metadata service so the same launch flow works
...
<Note>
  **Prerequisites**
  - A guest image with `cloud-init` (Linux) or `Cloudbase-Init` (Windows)
    pre-installed. The Polystack public image catalog ships with both.
  - A network or metadata route from the instance to the Polystack metadata
...
echo "$(date -u +%FT%TZ) cloud-init bootstrap complete" > /var/log/polystack-bootstrap.done
```
...
users:
  - name: ops
    ...
    shell: /bin/bash
    ssh_authorized_keys:
      - ssh-ed25519 AAAA... ops@polystack

write_files:
  - path: /etc/polystack/site.conf
    permissions: "0644"
    content: |
      backend = production
      region  = ap-south-1
...
runcmd:
  - systemctl enable --now nginx
  - curl -fsSL https://repo.polystack.tech/setup.sh | bash
  - echo "bootstrap-done" > /var/log/polystack-bootstrap.done

final_message: "Polystack bootstrap finished after $UPTIME seconds"
```
...
Set-Content -Path C:\inetpub\wwwroot\index.html `
  -Value "<h1>Polystack-managed IIS host</h1>" -Encoding UTF8

# Disk-level marker the monitoring agent picks up
$now = Get-Date -Format "yyyy-MM-ddTHH:mm:ssZ"
"$now Polystack bootstrap complete" | Out-File C:\polystack-bootstrap.done -Encoding UTF8
```
...
nltest /dsregdns
echo Polystack bootstrap complete > C:\polystack-bootstrap.done
```
...
— or on instances that were imported into Polystack through migration or
      ...
      tasks:
        - name: Rotate the deploy key
          ansible.builtin.shell: |
            sudo /opt/polystack/rotate-key.sh
          when: ansible_os_family != "Windows"

        - name: Rotate the deploy key on Windows hosts
          ansible.windows.win_powershell:
            script: C:\Program Files\Polystack\rotate-key.ps1
          when: ansible_os_family == "Windows"
    ```

    Ironcore Deployment Automation already includes the inventory plugin
...
<AccordionGroup>
  <Accordion title="The script never ran">
    ...
    or pick one from the Polystack public catalog.
  </Accordion>
...
- **Do not embed secrets in user-data.** The metadata service is reachable
  by every process inside the guest. Use Polystack Key Management for
...
- **Network reachability of the metadata service.** Restrict the
  ...
  after first boot.
- **Audit script content.** Ironcore Deployment Automation's `--check`
...
<CardGroup cols={3}>
  ...
  <Card title="Windows VM Guide" icon="desktop" href="/services/compute/windows-vm-guide">
  ...
  <Card title="Instance Rollback" icon="rotate-left" href="/services/compute/instance-rollback">
