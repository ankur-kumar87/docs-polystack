...
<Note>
  **Prerequisites**
  ...
  - the deployment console access for host-level configuration changes
...
<AccordionGroup>
  <Accordion title="Restrict metadata access via security groups" icon="shield">
    ...
    Configure metadata access restrictions through the deployment console under **Compute → Advanced
  ...
  <Accordion title="Enable metadata service authentication" icon="key">
    ...
    Enable authenticated metadata through the deployment console under **Compute → Advanced Settings →
  ...
  <Accordion title="Apply API rate limiting on the metadata endpoint" icon="gauge">
    ...
    Configure endpoint rate limits through the deployment console under **Compute → Security →
...
<AccordionGroup>
  <Accordion title="Enable live migration TLS" icon="lock">
    ...
    Enable TLS through the deployment console under **Compute → Security → Live Migration TLS**.
...
<AccordionGroup>
  <Accordion title="Configure rate limits" icon="gauge">
    ...
    configuration through the deployment console under **Compute → Security → API Rate Limiting**.
  </Accordion>
  ...
  <Accordion title="Enable automatic temporary bans" icon="ban">
    ...
    Enable automatic bans through the deployment console under **Compute → Security → API Rate
    ...
    <Tip>
      ...
      in Polystack Monitoring for sustained `429` error rates above 1% of total
      API traffic.
    </Tip>
  </Accordion>
</AccordionGroup>
...
|---------|---------------|---------|
| Metadata service authentication enabled | the deployment console → Compute → Metadata Security | High |
| Metadata endpoint rate limiting applied | the deployment console → Compute → Metadata Rate Limiting | High |
| Live migration TLS enabled | the deployment console → Compute → Security → Live Migration TLS | High |
| API rate limiting configured | the deployment console → Compute → Security → API Rate Limiting | Medium |
