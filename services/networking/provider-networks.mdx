...
description: "Configure VLAN, flat, and VXLAN provider networks in Polystack. Map physical network interfaces to tenant segmentation types for data center integration."
...
<Note>
  **Prerequisites**
  ...
  - the deployment console access for cluster-level interface configuration
...
via the deployment console. These settings apply to all cluster nodes.

<Tabs>
  <Tab title="Web Console" icon="rocket">
    <Steps titleSize="h3">
      <Step title="Open Networking configuration" icon="compass">
        In the deployment console, navigate to **Configuration → Networking**.
      </Step>
      <Step title="Configure physical interface mapping" icon="gear">
...
<Tabs>
  <Tab title="CLI" icon="terminal">
    <Steps titleSize="h3">
      ...
      <Step title="Create a VLAN provider network" icon="layer-group">
      ...
      <Step title="Create a VXLAN network" icon="layer-group">
