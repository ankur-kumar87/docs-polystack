...
description: "Understand the Polystack K8SaaS architecture — component roles, provisioning flow, infrastructure dependencies, and Conductor lifecycle management."
sidebarTitle: "Architecture"
---

## Overview

Polystack Kubernetes as a Service (K8SaaS) automates the full lifecycle of Kubernetes clusters
on top of Polystack infrastructure services. The platform is built around a Conductor that
orchestrates cluster create, update, delete, and upgrade operations by composing calls to
Compute, Load Balancer, DNS, and Networking services through Polystack Orchestration templates.
...
graph TD
    ...
    subgraph Provisioning
        ORCH[Polystack Orchestration<br/>Heat Templates]
    end

    subgraph Infrastructure
        COMPUTE[Polystack Compute<br/>VM instances]
        LB[Polystack Load Balancer<br/>Master API VIPs]
        DNS[Polystack DNS<br/>Cluster endpoints]
        NET[Polystack Networking<br/>Tenant subnets]
    end
...
<AccordionGroup>
  ...
  <Accordion title="Conductor" icon="microchip">
  ...
  <Accordion title="Orchestration Templates" icon="layer-group">
...
sequenceDiagram
    ...
    participant ORCH as Polystack Orchestration
    participant COMPUTE as Polystack Compute
...
|---------|------|----------------|
| Polystack Compute | VM instances for master and worker nodes | 2025.1 |
| Polystack Orchestration | Stack management for cluster resources | 2025.1 |
| Polystack Load Balancer | API server VIP and Kubernetes service LBs | 2025.1 |
| Polystack Networking | Tenant subnet allocation for cluster nodes | 2025.1 |
| Polystack DNS | Endpoint records for ingress and services | Optional |
| Polystack Block Storage | Persistent volume claims for stateful workloads | 2025.1 |
| Polystack Key Management | Cluster CA private key storage (optional) | Optional |
...
<CardGroup cols={2}>
  <Card title="Cluster Drivers" href="/services/kubernetes/admin-guide/cluster-drivers" color="#bf9667">
