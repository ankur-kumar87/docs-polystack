...
description: "Replicate a running VMware virtual machine to Polystack with a full sync followed by continuous incremental block-level replication — minimal downtime at cutover."
sidebarTitle: "Warm Migration"
---

## Overview

Warm migration moves a workload from VMware to Polystack while the source VM
continues to run. XMS takes a snapshot on the source, copies the full disk
contents into a target Polystack Block Storage volume, and then replicates only
...
and starts the target instance on Polystack — typically within minutes.
...
<Note>
  **Prerequisites**
  ...
  - A target Polystack project with enough quota for the compute, memory, and
...
| **Completed** | Target instance is running on Polystack |
  ...
  <Tab title="Dashboard" icon="gauge">
    <Steps titleSize="h3">
      ...
      <Step title="Configure target" icon="folder">
        | Field | Description |
        |-------|-------------|
        | **Target Project** | Polystack project that will host the migrated instance |
        | **Instance Name** | Name the target instance will use after cutover |
        | **Flavor** | Polystack flavor to match or exceed the source shape |
      ...
      <Step title="Map networks and set sync schedule" icon="network">
        Map every source NIC to a target Polystack network and subnet. Choose a
        sync cadence:
...
Because every write targets the same offsets on the same Polystack Block Storage
...
- Proceed to [cutover](/services/migration/user-guide/cutover) when you are
  ready to switch the workload to Polystack
...
<CardGroup cols={3}>
  <Card title="Cutover" href="/services/migration/user-guide/cutover" color="#197560">
    Execute the final sync and switch the workload to Polystack
  </Card>
