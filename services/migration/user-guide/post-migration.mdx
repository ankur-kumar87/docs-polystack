...
description: "Verify a migrated workload boots, networks, stores, and behaves correctly on Polystack before retiring the source VM."
...
<Note>
  **Prerequisites**
  ...
  - Polystack Dashboard access to the target project
...
<Steps titleSize="h3">
  <Step title="Target instance reaches active power state" icon="power">
    In the Polystack Dashboard, open **Compute → Instances** in the target project
  ...
  <Step title="Boot completed and console is reachable" icon="monitor">
    Open the instance console from the Polystack Dashboard and confirm:
...
| **Hostname resolves but services not reachable** | Firewall inside the guest blocks the new IP range | Update the guest firewall rules to allow the target Polystack network range |
...
<CardGroup cols={3}>
  ...
  <Card title="Polystack Compute" href="/services/compute" color="#197560">
    Manage the migrated instance lifecycle on Polystack
  </Card>
