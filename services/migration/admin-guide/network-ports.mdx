---
title: "Network Ports"
description: "Network paths and ports that must be reachable between XMS, source environments, Polystack services, and target guests."
...
| XMS | Polystack Identity | 5000 | Token validation | Egress |
| XMS | Polystack Block Storage | 8776 | Volume create and write | Egress |
| XMS | Polystack Compute | 8774 | Instance create and attach | Egress |
| XMS | Polystack Networking | 9696 | Network mapping validation | Egress |
...
## XMS ↔ Polystack Services

XMS acts as an Polystack tenant on behalf of every migration. It makes standard
API calls against:
...
who run XMS alongside the rest of the Polystack platform typically do not need
...
same endpoint used for the rest of the Polystack Dashboard and CLI. The
...
After a migration completes, the target Polystack instance is a normal compute
instance. XMS does not inject any agent or open any extra port on the target
guest — standard Polystack network security group rules apply for egress,
...
<Steps titleSize="h3">
  ...
  <Step title="Verify internal Polystack service traffic" icon="check">
  ...
  <Step title="Verify target instance traffic" icon="shield">
    No change required — standard Polystack security group rules apply to the
    migrated instance.
  </Step>
</Steps>
