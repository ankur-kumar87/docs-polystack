...
description: "How XMS writes migrated disk data into Polystack Block Storage and the supported back-end configurations."
sidebarTitle: "Storage Back-ends"
---

## Overview

Every migration writes source disk data into Polystack Block Storage. XMS does
not manage storage directly — it uses the platform's block storage API, so
any back-end that Polystack Block Storage supports is usable as a migration
...
graph LR
    ...
    X -->|Block Storage API| BS[Polystack Block Storage]
    BS --> V[Target Volume]
```
...
snapshotted, backed up, or restored by the standard Polystack tooling.
...
If your Polystack platform has more than one storage back-end, you can direct
...
Because the target volume is a native Polystack block storage volume, you can:

- Snapshot it through the standard snapshot API once the migration is done
- Back it up through Polystack Block Storage backup if your deployment supports
  cross-backend backup
- Use it as the source for Polystack Disaster Recovery protection plans
...
<CardGroup cols={3}>
  ...
  <Card title="Block Storage" href="/services/storage/index" color="#197560">
    Polystack Block Storage user guide and operator reference
  </Card>
