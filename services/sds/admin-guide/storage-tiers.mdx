---
title: "Storage Tiers"
description: "Configure multi-tier storage in Software-Defined-Storage by mapping device class pools to Cinder volume types, enabling workload-appropriate media selection for NVMe, SSD, and HDD tiers."
...
through volume types in the block storage service and pool configurations in Software-Defined-Storage.
Tier configuration is managed through the deployment console's Storage Tiers panel, which
auto-generates the required pool, CRUSH rule, and block storage backend configuration.

<AdminWarning />

<Tip>
  **the deployment console GUI** — Storage tier detection, CRUSH rule creation, and pool configuration can be performed through the Software-Defined-Storage Storage interface under the **Storage Tiers** tab. Click **Detect Tiers** to automatically discover device classes. No manual file editing required.
</Tip>
...
## Configuring Tiers via the deployment console

<Steps titleSize="h3">
  <Step title="Open Storage Tiers panel" icon="layer-group">
    Navigate to **the deployment console → Configuration → Storage Tiers**.
  ...
  <Step title="Apply configuration" icon="check">
    Click **Apply**. the deployment console automatically:
...
For environments where the deployment console is not managing tier configuration, configure tiers manually.
...
<Tabs>
  ...
  <Tab title="Register Cinder Backend" icon="gear">
    Add the new backend to your Cinder configuration (managed by the deployment console via
    `globals.d/_50_ceph_tiers.yml`):

    ```yaml title="/etc/ironcore/globals.d/_50_ceph_tiers.yml"
    ...
    ironcore-ansible deploy -t cinder
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="Test Volume Creation" icon="flask">
...
<CardGroup cols={2}>
  ...
  <Card title="Block Storage — Volume Types" href="/services/storage/volume-types-admin" color="#bf9667">
