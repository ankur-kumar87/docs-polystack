...
description: "Diagnose and resolve Software-Defined-Storage cluster-level issues — HEALTH_WARN states, OSD failures, slow requests, PG inconsistencies, and capacity emergencies."
...
This page covers cluster-level troubleshooting procedures for Software-Defined-Storage administrators.
For tenant-facing storage issues (stuck volumes, snapshot failures), see the
[Software-Defined-Storage User Guide Troubleshooting](/services/sds/user-guide/troubleshooting) page.
...
<AccordionGroup>
  <Accordion title="HEALTH_WARN: too few PGs" icon="layer-group">
  ...
  <Accordion title="OSD down after hardware failure" icon="hard-drive">
    ...
    **Resolution**:
    ...
    5. Redeploy the OSD through the deployment console on the replacement device
  ...
  <Accordion title="Slow requests / high latency" icon="clock">
    ...
    **Resolution**:
    ...
    | CPU throttling on OSD nodes | Review CPU allocation in the deployment console |
  ...
  <Accordion title="PG inconsistent" icon="circle-xmark">
  ...
  <Accordion title="Pool near full / cluster full" icon="gauge">
    ...
    **Resolution**:
    ...
    - **Long-term**: Add new OSD nodes through the deployment console (see [Capacity Planning](/services/sds/admin-guide/capacity-planning))
...
<CardGroup cols={2}>
  ...
  <Card title="User Guide Troubleshooting" href="/services/sds/user-guide/troubleshooting" color="#bf9667">
