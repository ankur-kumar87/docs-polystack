...
description: "Monitor Software-Defined-Storage cluster utilization, maintain safe capacity headroom, and plan storage expansion before capacity constraints impact performance or availability."
...
Maintaining adequate free capacity in an Software-Defined-Storage cluster is critical for both performance
...
<Note>
  **Prerequisites**
  ...
  - Access to **the deployment console** (`https://connect.<your-domain>`) for node provisioning
</Note>
...
<Tabs>
  <Tab title="Dashboard" icon="gauge">
    Navigate to **the deployment console → Storage → Capacity** for a graphical capacity overview
...
<Steps titleSize="h3">
  <Step title="Deploy new OSD node via the deployment console" icon="server">
    Navigate to **the deployment console → Infrastructure → Nodes → Add Node** and register the
    new storage node. the deployment console configures the OS, installs storage packages, and
  ...
  <Step title="Monitor rebalancing" icon="arrows-rotate">
...
Configure Monitoring alerts to proactively notify administrators before capacity reaches
critical thresholds:

| Alert | Threshold | Monitoring Metric |
|-------|----------|------------|
| Capacity Warning | Pool used > 70% | `polystack_storage_pool_used_pct` |
| Capacity Critical | Pool used > 80% | `polystack_storage_pool_used_pct` |
| OSD Near Full | OSD used > 85% | `polystack_storage_osd_used_pct` |

Navigate to **Monitoring → Alerting → Alert Rules** in the Monitoring portal and create
rules sourcing from the `polystack_storage` metric namespace.
...
<CardGroup cols={2}>
  ...
  <Card title="Monitoring" href="/services/sds/admin-guide/monitoring" color="#bf9667">
    Configure Monitoring alerts for capacity and health thresholds
  </Card>
