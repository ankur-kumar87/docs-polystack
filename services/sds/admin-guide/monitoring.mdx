---
title: "Monitoring"
description: "Monitor XSDS cluster health, OSD status, and I/O performance through XIMP integration — key metrics, alert thresholds, and observability configuration."
...
<Steps titleSize="h3">
  ...
  <Step title="Add scrape target in XIMP" icon="plus">
    ...
    Or via CLI:
    ```bash title="Add XSDS scrape target"
    ximp target add \
      --url http://<MGMT_NODE_IP>:9283/metrics \
      --interval 60s \
      --label service=xsds \
