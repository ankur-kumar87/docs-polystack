...
and managed by the deployment console.

<Tip>
  **the deployment console GUI** — The distributed storage cluster can be bootstrapped and managed through the Software-Defined-Storage Storage interface. Storage tiers, CRUSH rules, and Ceph configuration are all accessible from the GUI. No manual file editing required.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="RADOS Gateway (RGW)" icon="globe">
    ...
    instances behind a load balancer for high availability. the deployment console configures the
