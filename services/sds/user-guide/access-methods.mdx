...
description: "Connect to Software-Defined-Storage block, object, and file storage through the Dashboard, CLI, S3-compatible API, and native mount protocols."
sidebarTitle: "Access Methods"
---

## Overview

Software-Defined-Storage exposes storage through multiple access protocols. The appropriate method depends
...
|---------------|--------------|--------------|
| **Polystack Dashboard** | Block, Object | GUI-based management and exploration |
...
<Tabs>
  <Tab title="Block Storage" icon="hard-drive">
    <Steps titleSize="h3">
      ...
      <Step title="Manage volumes" icon="gear">
...
The Software-Defined-Storage object storage service exposes a fully S3-compatible API. Any application or
tool that supports the AWS S3 API can connect to Software-Defined-Storage without modification.

<Tabs>
  <Tab title="Generate Credentials" icon="key">
    <Steps titleSize="h3">
      <Step title="Navigate to Access Keys" icon="key">
        In the Polystack Dashboard, navigate to **Project → Object Store → Access Keys**
        and click **Create Key**.
      </Step>
      <Step title="Save your credentials" icon="file-lines">
  ...
  <Tab title="Python (boto3)" icon="code">
    ...
    s3 = boto3.client(
        ...
        region_name='polystack-region-1'
    )
  ...
  <Tab title="CLI (aws s3)" icon="terminal">
    ...
    aws configure --profile polystack
    # AWS Access Key ID: YOUR_ACCESS_KEY
    # AWS Secret Access Key: YOUR_SECRET_KEY
    # Default region name: polystack-region-1
    # Default output format: json
    ```

    ```bash title="List buckets"
    aws s3 ls --endpoint-url https://object.<your-domain> --profile polystack
    ```
...
<Tabs>
  <Tab title="NFS (Linux)" icon="terminal">
    <Steps titleSize="h3">
      ...
      <Step title="Persist mount across reboots" icon="file-lines">
        Add to `/etc/fstab`:
  ...
  <Tab title="SMB (Windows)" icon="desktop">
    <Steps titleSize="h3">
      ...
      <Step title="Map network drive" icon="hard-drive">
        ...
        Enter the UNC path and provide your Polystack credentials when prompted.
...
<CardGroup cols={2}>
  ...
  <Card title="Performance" href="/services/sds/user-guide/performance" color="#bf9667">
