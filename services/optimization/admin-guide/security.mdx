---
title: "Security"
description: "Secure the Xloud Optimization — configure the Applier service account, restrict RBAC execution policies, enable API audit logging, and protect the."
...
<Tabs>
    ...
    <Steps titleSize="h3">
      ...
      <Step title="Custom Authentication Settings (Optional)" icon="settings">
        ...
        **Advanced Configuration**. In the **Service Tree** (left panel), select **watcher**.
        ...
        ```ini title="/etc/xavs/config/watcher/watcher.conf"
        [keystone_authtoken]
  ...
  <Tab title="CLI" icon="terminal">
    ...
    ```ini title="/etc/xavs/watcher/watcher.conf -- keystone_authtoken section"
    [keystone_authtoken]
    www_authenticate_uri = http://10.0.1.71:5000
    auth_url = http://10.0.1.71:5000/v3
    memcached_servers = 10.0.1.71:11211
...
```yaml title="/etc/xavs/watcher/policy.yaml — execution override"
...
```ini title="/etc/xavs/watcher/watcher.conf — audit logging"
[DEFAULT]
use_journal = True
