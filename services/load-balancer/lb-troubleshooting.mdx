...
<AccordionGroup>
  <Accordion title="Load balancer API service unavailable" icon="server" defaultOpen>
    ...
    **Resolution**: Restart services via the deployment console:
    ```bash title="Restart load balancer services"
    ironcore-ansible deploy --tags octavia
    ```
  </Accordion>

  <Accordion title="RabbitMQ connectivity lost" icon="comment">
...
<AccordionGroup>
  <Accordion title="Appliance provisioning fails — DHCP pool exhausted" icon="network-wired">
  ...
  <Accordion title="Appliance image unavailable" icon="compact-disc">
