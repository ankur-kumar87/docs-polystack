...
<Note>
  **Prerequisites**
  ...
  - the deployment console access for applying configuration changes
</Note>
  ...
  <Tab title="Web Console" icon="settings">
    ...
    <Steps titleSize="h3">
      <Step title="Open Advanced Configuration">
        Navigate to **the deployment console > Advanced Configuration** and select **skyline-apiserver**
        in the service tree.
      </Step>
...
<AccordionGroup>
  ...
  <Accordion title="Token expiration">
    ...
    Configure Keystone token expiration through the deployment console to adjust session duration.
  </Accordion>
