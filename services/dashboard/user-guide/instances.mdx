...
The Polystack compute layer keeps a single source of truth for every running
virtual machine. Whether an instance was launched through the Dashboard, CLI,
infrastructure-as-code, orchestration, or a custom SDK workflow, the instance
appears in **Compute → Instances** automatically.

| Where the VM came from | Visible in Compute → Instances? |
|------------------------|:--------------------------------:|
| Dashboard — Create Instance wizard | Yes |
