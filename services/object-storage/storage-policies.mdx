...
<AccordionGroup>
  <Accordion title="Rclone — universal multi-cloud sync" icon="refresh-cw">
    ...
    ```bash title="Configure Rclone for Xloud Object Storage"
    ...
    ```bash title="Sync from AWS S3 to Xloud Object Storage"
    rclone sync s3:my-aws-bucket xloud-swift:my-local-container
    ```

    ```bash title="Mount Xloud container as local filesystem"
  ...
  <Accordion title="AWS CLI — S3-compatible access" icon="cloud">
    ...
    ```bash title="Configure AWS CLI for Xloud S3 endpoint"
    ...
    ```bash title="Sync local directory to Xloud"
    aws s3 sync ./backup/ s3://my-backup-bucket \
      --endpoint-url https://object.<your-domain>
    ```
  </Accordion>
  <Accordion title="Python boto3 — programmatic access" icon="code">
    ```python title="boto3 client for Xloud S3-compatible endpoint"
    import boto3
