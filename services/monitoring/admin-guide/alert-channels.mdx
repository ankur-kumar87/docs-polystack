...
description: "Configure Monitoring notification channels — email, webhook, PagerDuty, and Slack — and set up multi-tier escalation policies for critical infrastructure alerts."
...
Alert channels define how Monitoring delivers notifications when alert rules fire. Channels
...
<Tabs>
  <Tab title="Web Console" icon="server">
    SMTP email alerts and webhook notifications can be configured directly in the deployment console:
    ...
    <Steps titleSize="h3">
      ...
      <Step title="Test the channels" icon="flask">
  ...
  <Tab title="CLI" icon="terminal">
    Alert channels can also be configured through the Monitoring CLI and the Monitoring
...
<Tabs>
  <Tab title="Dashboard" icon="gauge">
    <Steps titleSize="h3">
      ...
      <Step title="Configure the channel" icon="gear">
      ...
      <Step title="Test the channel" icon="flask">
  ...
  <Tab title="CLI" icon="terminal">
    ...
    monitoring alert channel list
    ```

    ```bash title="Add email channel"
    monitoring alert channel create \
      --name ops-email \
      --type email \
    ...
    monitoring alert channel test ops-webhook
    ```

    ```bash title="Delete a channel"
    monitoring alert channel delete ops-webhook
    ```
  </Tab>
</Tabs>
...
<Steps titleSize="h3">
  ...
  <Step title="Define escalation tiers" icon="layer-group">
...
<AccordionGroup>
  <Accordion title="Email (SMTP)" icon="envelope">
  ...
  <Accordion title="Slack" icon="comment-dots">
...
<CardGroup cols={2}>
  <Card title="Metrics & Alerts (User Guide)" href="/services/monitoring/user-guide/metrics-alerts" color="#bf9667">
