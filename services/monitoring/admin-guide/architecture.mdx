...
<Tabs>
  <Tab title="XDeploy" icon="server">
    Monitoring services are enabled and configured through the XDeploy Configuration panel:
...
<AccordionGroup>
  ...
  <Accordion title="Log Collector" icon="file-text">
...
<Tabs>
  ...
  <Tab title="Scaled (Multi-Node Monitoring)" icon="layers">
    ...
    XDeploy manages the multi-node Monitoring deployment. Navigate to
