---
title: "Log Analytics"
description: "Search, filter, and analyze centralized log streams in Monitoring. Create log-based alert rules for real-time event detection across your infrastructure."
sidebarTitle: "Log Analytics"
---

## Overview

Monitoring aggregates log streams from all registered infrastructure nodes and services
...
<Note>
  **Prerequisites**
  ...
  - Log collection configured for the services you want to query
    (see [Monitoring Admin — Log Collection](/services/monitoring/admin-guide/log-collection))
</Note>
...
<Tabs>
  <Tab title="Dashboard" icon="gauge">
    <Steps titleSize="h3">
      <Step title="Open Log Explorer" icon="magnifying-glass">
  ...
  <Tab title="CLI" icon="terminal">
    ...
    monitoring log tail --query 'level:ERROR'
    ```
  </Tab>
</Tabs>
...
<AccordionGroup>
  <Accordion title="Find all errors in the last hour" icon="circle-xmark">
    ```
    level:ERROR
    ```
  ...
  <Accordion title="Network connection refused" icon="network-wired">
    ```
...
<CardGroup cols={2}>
  ...
  <Card title="Monitoring Admin — Log Collection" href="/services/monitoring/admin-guide/log-collection" color="#bf9667">
