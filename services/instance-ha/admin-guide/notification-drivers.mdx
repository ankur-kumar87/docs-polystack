...
structured Instance HA notifications, and route them to the Recovery Engine. Polystack Instance
...
|--------|--------|----------|-------------|
| `NovaNotificationDriver` | Polystack Compute message bus | AMQP | Default — all standard deployments |
...
The `NovaNotificationDriver` is enabled by default in all Polystack Instance HA deployments.
It subscribes to the Polystack Compute AMQP message bus and listens for `compute.host.error`
...
<AccordionGroup>
  <Accordion title="How it works" icon="bell" defaultOpen>
    ...
    standard the deployment console deployment.
  </Accordion>
  <Accordion title="Verify the driver is active" icon="circle-check">
    ...
    grep -i notification_driver \
      /etc/ironcore/instance-ha/instance-ha.conf
    ```
...
{
  "hostname": "compute-01.polystack.local",
  "type": "COMPUTE_HOST",
  "payload": {
    "event": "STOPPED",
...
<Warning>
  The Instance HA API uses Polystack Identity token authentication. Generate a service
  ...
  <Tab title="Web Console" icon="gauge">
    <Steps titleSize="h3">
      ...
      <Step title="Edit workflow targets" icon="file-code">
        ...
        ```ini title="TaskFlow workflow in the deployment console Advanced Configuration"
