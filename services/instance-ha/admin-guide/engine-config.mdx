...
<Tabs>
  <Tab title="Web Console" icon="gauge">
    <Steps titleSize="h3">
      <Step title="Enable Host HA" icon="toggle-right">
        In the deployment console, navigate to **Configuration → Advance Features** and toggle
        **Enable Host HA** to **Yes**. Click **Save Configuration**.

        <Info>
          Database connection strings (`[database]`) and Polystack Identity credentials
          (`[keystone_authtoken]`) are **auto-managed** by the deployment console. Do not edit these
      ...
      <Step title="Customize engine parameters (optional)" icon="file-code">
        ...
        **Advanced Configuration** in the deployment console. In the **Service Tree**, select
        **masakari** and open (or create) `instance-ha.conf`.

        Edit the parameters in the Code Editor:

        ```ini title="Engine parameters in the deployment console Advanced Configuration"
        [DEFAULT]
        host = controller-01
  ...
  <Tab title="CLI" icon="terminal">
    ...
    <Warning>
      ...
      configuration entirely through CLI without the deployment console.
    </Warning>
  </Tab>
</Tabs>
...
<AccordionGroup>
  ...
  <Accordion title="Faster recovery for latency-sensitive workloads" icon="bolt">
  ...
  <Accordion title="Enable recovery for instances in ERROR state" icon="circle-xmark">
