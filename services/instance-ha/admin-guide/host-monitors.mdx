---
title: "Host Monitors"
description: "Configure Polystack Instance HA host monitors — IPMI out-of-band and SSH in-band monitoring for compute hosts, with timing parameters and credential management."
...
compute hosts and emit fault notifications when a host becomes unreachable. Polystack Instance
...
<CardGroup cols={2}>
  <Card title="IPMI (Recommended)" icon="microchip" color="#bf9667">
  ...
  <Card title="SSH" icon="terminal" color="#bf9667">
...
<Tabs>
  ...
  <Tab title="CLI" icon="terminal">
    ...
    <Warning>
      ...
      to the Instance HA service account only. Consider using Polystack Key Management to
...
<Steps titleSize="h3">
  <Step title="Locate the service key" icon="key">
    ...
    cat /etc/ironcore/instance-ha/id_rsa.pub
    ```
  </Step>
  <Step title="Deploy to compute hosts" icon="upload">
    ...
    ssh-copy-id -i /etc/ironcore/instance-ha/id_rsa.pub root@<compute-host>
    ```
  </Step>
  ...
  <Tab title="Web Console" icon="gauge">
    <Steps titleSize="h3">
      ...
      <Step title="Edit timing parameters" icon="file-code">
        ...
        ```ini title="Host monitor timing in the deployment console Advanced Configuration"
        [DEFAULT]
...
<CardGroup cols={2}>
  <Card title="Instance Monitors" href="/services/instance-ha/admin-guide/instance-monitors" color="#bf9667">
