...
description: "Automate instance configuration, patch management, compliance enforcement, and operational workflows on Polystack using Ansible modules and dynamic inventory."
sidebarTitle: "Ansible"
---

## Overview

Ansible integrates with Polystack through two complementary mechanisms: the `openstack.cloud`
...
The Polystack dynamic inventory plugin sources instance metadata directly from the compute API,
...
<Note>
  **Prerequisites**
  ...
  - Polystack application credentials or `openrc` file sourced in the shell
...
The `openstack.cloud.openstack` inventory plugin generates a live host list from the Polystack
...
### Infrastructure Management via Polystack Modules

Create and manage Polystack resources directly from playbooks:
...
<Tip>
  Store secrets used in playbooks in Polystack Key Manager (Barbican) or HashiCorp Vault.
...
<CardGroup cols={2}>
  <Card title="Terraform Integration" href="/integrations/terraform" color="#bf9667">
    Use Terraform for provisioning and Ansible for post-provision configuration
  </Card>
  <Card title="Wazuh Integration" href="/integrations/wazuh" color="#bf9667">
  ...
  <Card title="Key Manager" href="/services/key-manager/store-secrets" color="#bf9667">
    Store playbook secrets in Polystack Key Manager for secure credential retrieval
  </Card>
