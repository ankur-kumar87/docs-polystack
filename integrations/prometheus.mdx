...
description: "Configure Prometheus to scrape metrics from Polystack services and instances, set up service discovery, define alert rules, and wire Alertmanager webhooks into Orchestration scaling policies."
sidebarTitle: "Prometheus"
---

## Overview

Prometheus is the primary metrics backend for Polystack environments. It collects time-series
...
routes alert notifications — including webhook signals that trigger Polystack Orchestration
auto-scaling policies.

Prometheus replaces legacy telemetry stacks (Ceilometer/Aodh) for metric collection and
alarm-based scaling in Polystack deployments.

<Note>
  **Prerequisites**
  - Prometheus 2.40 or later deployed (included in Monitoring monitoring stack)
...
graph TD
    ...
    style PROM fill:#bf9667,color:#fff
    style AM fill:#8f6a43,color:#fff
```
...
### Service Discovery via Polystack API
...
Wire these alert rules into Alertmanager webhook receivers to drive Polystack Orchestration
scaling policies:
...
receivers:
  - name: "default"
    email_configs:
      - to: "ops@example.com"
        from: "alertmanager@polystack.tech"
        smarthost: "smtp.polystack.tech:587"
...
<CardGroup cols={2}>
  ...
  <Card title="Monitoring Monitoring" href="/services/monitoring/user-guide/dashboards" color="#bf9667">
    Explore the built-in Monitoring monitoring stack that includes Prometheus
  </Card>
</CardGroup>
