...
<Tabs>
  <Tab title="Ironcore Settings" icon="settings">
    ...
    | **External VIP Address** | No | Public-facing VIP for external access and the Polystack Dashboard. Required only if external access is needed. |
    ...
    | **Internal FQDN** | Hostname for internal API endpoints (e.g., `ironcore-int.example.com`). Must resolve to the Internal VIP. |
    | **External FQDN** | Hostname for external access (e.g., `ironcore.example.com`). Must resolve to the External VIP. |
    ...
    Certificates are saved to `/etc/ironcore/certificates/` on the deployment node and distributed to all controllers during deployment.
  ...
  <Tab title="Network" icon="network">
    Configures the Polystack Networking service --- interfaces, provider networks, and optional networking features.
  ...
  <Tab title="Storage" icon="hard-drive">
    Configures Polystack Block Storage --- the persistent volume service for VM disks.
    ...
    <Tip>
      For production multi-node deployments, Polystack recommends **Ceph RBD**. It provides data replication across nodes, eliminating single points of failure for VM disks.
    </Tip>
  </Tab>

  <Tab title="Load Balancer" icon="network">
    Configures the Polystack Load Balancer service for distributing network traffic across backend instances.
  ...
  <Tab title="Advance Features" icon="layers">
    ...
    | Feature | Default | Description |
    |---------|---------|-------------|
    | **Enable KMS** | OFF | When enabled, the Polystack Key Management service (Barbican) is deployed for secret storage, encryption keys, and certificate management |
  ...
  <Tab title="Custom Config" icon="terminal">
    ...
    <Tip>
      Use Custom Configuration as an escape hatch. There are hundreds of deployment variables available in the underlying automation --- the other tabs cover the most common ones. Consult the [ironcore-ansible variable reference](https://docs.polystack.tech) for the full list.
    </Tip>
...
<CardGroup cols={2}>
  <Card title="Container Images" icon="package" href="/deployment/images" color="#bf9667">
