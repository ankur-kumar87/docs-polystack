...
description: "Complete deployment and lifecycle management for your Polystack cloud infrastructure"
icon: "play"
...
<Note>
  **Prerequisites** — Before starting any deployment workflow, ensure you have:
  ...
  - A valid XDeploy activation key from [license.polystack.tech](https://license.polystack.tech)
</Note>
...
<div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
  <div>
    End-to-end XDEPLOY walkthrough — activation, hosts, Software-Defined-Storage storage, cluster configuration, load balancer, monitoring, advanced features, cluster licensing, and the Operations workflow that brings the cluster live.
...
<Steps titleSize="h3">
  ...
  <Step title="Software-Defined-Storage Bootstrap" icon="database">
    Deploy the distributed storage cluster. **Skip this step if not using Software-Defined-Storage as your storage backend.**
  </Step>
...
<Steps titleSize="h3">
  <Step title="Install XOS on All Nodes" icon="monitor">
    ...
    <Info>XOS is available as an ISO from your Polystack representative or the [license portal](https://license.polystack.tech). See the [XOS product page](/products/xos) for details.</Info>
  </Step>
  ...
  <Step title="Plan Your Network" icon="network">
    ...
    | Storage Client (Software-Defined-Storage) | Client-to-Ceph I/O | Recommended |
    | Storage Replication (Software-Defined-Storage) | Ceph OSD-to-OSD replication | Recommended |
  ...
  <Step title="Gather Credentials" icon="key">
    ...
    - **XDeploy activation key** (`.xlic`) from [license.polystack.tech](https://license.polystack.tech)
    - **Cluster license key** (`.xlic`) from [license.polystack.tech](https://license.polystack.tech)
  </Step>
  <Step title="Choose Architecture and Node Roles" icon="layout">
    ...
    <CardGroup cols={3}>
      <Card title="All-in-One (AIO)" icon="square" color="#bf9667">
        Single server runs all roles. For labs, demos, and development. Minimum 1 node.
      </Card>
      <Card title="Hyper-Converged (HCI)" icon="layers" color="#bf9667">
        Each node runs compute + storage together. Reduces hardware count. Minimum 3 nodes for HA.
      </Card>
      <Card title="Distributed" icon="grid" color="#bf9667">
        Dedicated nodes per role — separate controller, compute, and storage. Maximum performance and isolation. Minimum 5+ nodes.
      </Card>
    </CardGroup>

    <Tip>For production environments, Polystack recommends HCI with a minimum of 3 nodes or Distributed with 5+ nodes for full high availability.</Tip>
  </Step>
</Steps>
...
<CardGroup cols={3}>
  <Card title="Bootstrap" icon="circle-check" href="/deployment/bootstrap" color="#bf9667">
  ...
  <Card title="Advanced Config" icon="file-text" href="/deployment/advanced-config" color="#bf9667">
    Per-service configuration file editor with version tracking
  </Card>
  <Card title="Images" icon="package" href="/deployment/images" color="#bf9667">
  ...
  <Card title="Cluster License" icon="key" href="/deployment/cluster-license" color="#bf9667">
    Cluster licensing and activation
  </Card>
  <Card title="XDeploy Key" icon="fingerprint" href="/deployment/xdeploy-key" color="#bf9667">
