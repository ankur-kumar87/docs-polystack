...
<Note>
  **Prerequisites**
  ...
  - Access to [license.polystack.tech](https://license.polystack.tech) for license file submission
</Note>
...
<CardGroup cols={2}>
  <Card title="License Overview" icon="shield-check" color="#bf9667">
  ...
  <Card title="Cluster Nodes" icon="server" color="#bf9667">
...
<Steps titleSize="h3">
  ...
  <Step title="Submit Request" icon="upload">
    Upload the `.xlic` request file to [license.polystack.tech](https://license.polystack.tech). After validation, you receive a signed license file in return.
...
The license controls which features are available in the Polystack Dashboard and enforces node count limits. Without a valid license, the cloud infrastructure runs with restricted functionality.
...
<AccordionGroup>
  <Accordion title="Verification fails after hardware replacement" icon="alert-triangle">
    ...
    **Resolution**: Generate a new license request that includes the updated hardware fingerprint. Upload the new request to [license.polystack.tech](https://license.polystack.tech) and install the new signed license file.
  </Accordion>
