...
description: "Step-by-step pre-deployment security hardening for Xloud Platform — OS hardening, SSH configuration, service minimization, database and message queue hardening."
...
<Steps titleSize="h3">
  ...
  <Step title="Harden the metadata service" icon="server">
    ...
    ```yaml title="/etc/xavs/globals.d/_60_metadata.yml"
...
<Steps titleSize="h3">
  ...
  <Step title="Enable binary log encryption" icon="database">
    ```yaml title="/etc/xavs/globals.d/_60_db_security.yml"
...
<Steps titleSize="h3">
  <Step title="Enable RabbitMQ TLS" icon="lock">
    ```yaml title="/etc/xavs/globals.d/_60_rabbitmq.yml"
