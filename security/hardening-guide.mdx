...
Hardening operates at four levels: the host operating system (Polystack OS), the Polystack platform services, supporting infrastructure components (database, message queue), and the metadata service.


<Note>
  **Prerequisites**
  - Polystack OS installed on all control and compute nodes
  - the deployment console access with bootstrap credentials
...
<Steps titleSize="h3">
  ...
  <Step title="Enable automatic security updates" icon="rotate-right">
  ...
  <Step title="Disable unused kernel modules" icon="microchip">
  ...
  <Step title="Configure kernel security parameters" icon="gear">
  ...
  <Step title="Enable AppArmor enforcement" icon="shield-halved">
    AppArmor profiles for hypervisor processes ship in enforce mode on Polystack OS. Verify and enable:
...
<Steps titleSize="h3">
  <Step title="Minimize running services" icon="circle-xmark">
  ...
  <Step title="Set file permission baselines" icon="file-shield">
...
<CardGroup cols={2}>
  ...
  <Card title="Troubleshooting" href="/security/hardening-guide" color="#bf9667">
