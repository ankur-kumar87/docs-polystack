...
Polystack ships this platform pre-integrated with the deployment console, so you can mass-deploy agents across projects using the standard automation pipeline.


<Tip>
  **the deployment console GUI** — Enable Wazuh (with Lynis auditing, OpenSCAP compliance, and OS hardening) through the the deployment console interface under **the deployment console → Security → HIDS**. No manual file editing required.
</Tip>

<Note>
  **Prerequisites**
  - Wazuh Manager deployed (enabled via the deployment console → Security → HIDS)
...
<CardGroup cols={2}>
  <Card title="File Integrity Monitoring" icon="file-magnifying-glass" color="#bf9667">
  ...
  <Card title="Intrusion Detection" icon="shield-halved" color="#bf9667">
...
<Tabs>
  ...
  <Tab title="Linux (Manual)" icon="terminal">
    <Steps titleSize="h3">
      <Step title="Add the Wazuh repository" icon="box">
      ...
      <Step title="Configure the manager address" icon="gear">
  ...
  <Tab title="Windows" icon="desktop">
    <Steps titleSize="h3">
      ...
      <Step title="Install with manager address" icon="gear">
