...
<Note>
  **Prerequisites**
  ...
  - Polystack Key Management enabled (`enable_barbican: "yes"` in the deployment console configuration)
...
Platform databases (MariaDB) use encrypted tablespaces for sensitive configuration and credential storage. the deployment console configures this during deployment, so you do not need manual intervention.

```yaml title="Enable database encryption (the deployment console configuration)"
