...
Polystack bundles Lynis in Polystack OS and makes it available via the the deployment console automation pipeline for both individual node audits and fleet-wide compliance sweeps.


<Note>
  **Prerequisites**
  - SSH access to the target host (or run directly on the node)
  - Lynis installed (pre-installed on Polystack OS nodes; install via `apt install lynis` on guest VMs)
...
<Tabs>
  <Tab title="Single Host" icon="terminal">
    <Steps titleSize="h3">
      ...
      <Step title="Review the hardening index" icon="chart-bar">
      ...
      <Step title="Review warnings and suggestions" icon="circle-xmark">
  ...
  <Tab title="Non-Interactive (CI/CD)" icon="gear">
  ...
  <Tab title="Fleet Audit (Ansible)" icon="layer-group">
...
<AccordionGroup>
  ...
  <Accordion title="Kernel parameters not hardened" icon="microchip">
  ...
  <Accordion title="World-writable files detected" icon="file">
