...
<Note>
  **Prerequisites**
  - the deployment console access with administrator privileges
  - SSL/TLS certificates (CA-signed, self-signed, or Let's Encrypt)
  - Access to the the deployment console configuration interface or `/etc/ironcore/globals.d/` on the bootstrap node
</Note>
...
<Tabs>
  <Tab title="Web Console" icon="gauge">
    <Steps titleSize="h3">
      <Step title="Open Global Configuration" icon="gear">
        Log in to the deployment console and navigate to **Configuration → Global Settings**.
      </Step>
      ...
      <Step title="Apply Configuration" icon="play">
        Click **Save and Deploy**. the deployment console runs the TLS configuration playbook across all nodes.
...
<Steps titleSize="h3">
  <Step title="Generate a CSR" icon="file-lines">
...
<Tabs>
  <Tab title="Dashboard" icon="gauge">
    Navigate to the the deployment console **Services** view. All service health indicators should show green. Click on any service to view its TLS status.
...
<CardGroup cols={2}>
  ...
  <Card title="API Security" href="/security/api-security" color="#bf9667">
