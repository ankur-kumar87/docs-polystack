...
Polystack Platform ships OpenSCAP tooling on Polystack OS and supports fleet-wide compliance scanning via the the deployment console automation pipeline. You can forward scan reports to SIEM systems or store them as audit artifacts for regulatory reviews.


<Note>
  **Prerequisites**
  - `openscap-scanner` and `scap-security-guide` packages installed (pre-installed on Polystack OS nodes)
...
<Tabs>
  ...
  <Tab title="With Remediation" icon="wrench">
    ...
    <Steps titleSize="h3">
      <Step title="Generate remediation playbook" icon="layer-group">
      ...
      <Step title="Re-scan to verify" icon="arrows-rotate">
  ...
  <Tab title="Fleet Scan (Ansible)" icon="layer-group">
...
<AccordionGroup>
  <Accordion title="Which profile should I use?" icon="circle-question">
  ...
  <Accordion title="Custom profile development" icon="gear">
...
<CardGroup cols={2}>
  ...
  <Card title="Lynis Auditing" href="/security/lynis" color="#bf9667">
    Run OS security audits with hardening index scoring
  </Card>
  <Card title="Compliance Frameworks" href="/security/compliance" color="#bf9667">
