...
<Note>
  **Prerequisites**
  ...
  - For FWaaS: platform license with `enable_neutron_fwaas: "yes"` in the deployment console configuration
  - For VPNaaS: `enable_neutron_vpnaas: "yes"` in the deployment console configuration
</Note>
...
For infrastructure-level DDoS protection, use the Monitoring monitoring integration to detect anomalous traffic patterns and trigger automated response playbooks.
...
<CardGroup cols={2}>
  ...
  <Card title="Networking Service" href="/services/networking/index" color="#bf9667">
